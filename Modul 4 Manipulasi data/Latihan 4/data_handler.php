		...
		if(isset($_GET['act']) && $_GET['act'] != '') {
			switch ($_GET['act']) {
				...
				case 'del':
					if (isset($_GET['id']) && ctype_digit($_GET['id'])) {
						$id = $_GET['id'];
						$ok = "DELETE FROM data WHERE nim='$id'"; 
						$res = mysql_query($ok);
						if ($res) {
...
function show_admin_data($root) { ?>
	...
	<?php
	$sql = 'SELECT nim, nama, alamat FROM data';
	$res = mysql_query($sql);
...
function data_detail($root, $id) {
	$sql = "SELECT nim, nama, alamat FROM data WHERE nim ='$id'";
	$res = mysql_query($sql);
	if($res) {
		if (mysql_num_rows($res)) { ?>
			<div class="tabel">
...
function data_editor($root, $id = 0) {
	...
	if(isset($_POST['nim']) && $_POST['nim']) {
		if (!$id) {
			...
			$nama 	= $_POST['nama'];
			$alamat = $_POST['alamat'];
			$res = mysql_query("INSERT INTO data VALUES ('".$nim."', '" .$nama."', '" .$alamat."')");
			if($res) { ?>
		...
		<?php
			...
			$nama 	= $_POST['nama'];
			$alamat = $_POST['alamat'];
			$res = mysql_query("UPDATE data SET nim='$nim', nama='$nama', alamat='$alamat' WHERE nim='$id'");
			if ($res) { ?>
